...
We introduce an integer parameter `STORAGE_POINT_PROP` initialized by `10^18`. The propotion of *p* is defined by `STORAGE_POINT_PROP / (STORAGE_POINT_PROP + 10^18)`. For example, if in the first voting period, all the votes agree to increase the `STORAGE_POINT_PROP`, `STORAGE_POINT_PROP` will become `2 * 10^18` and *p* equals 67% (calculated in the same way as in [CIP-94](./cip-94.md#specification)).
