...
- [Copyright](#copyright)
- [Appendix 1: Implementation and Security Recommendations for Custodial Wallet Providers](#appendix-1-implementation-and-security-recommendations-for-custodial-wallet-providers)
...
This CIP proposes a set of Provider RPC APIs for hosted wallets.

## Abstract

Wallets are the entry point for users to access blockchain. Depending on the implementation and key storage method, different types of wallets have significant differences. Among them, hosted wallets are a type of wallet service: the user's private key is stored on a third-party server, and the user uses the wallet by using the services of the hosted wallet provider. This type of wallet introduces trust in the wallet provider, bringing certain risks, but also reduces the threshold for using the blockchain: hosted wallets can shield users from key management details, and also allow users to interact with DAPPs without installing browser plugins (or wallet clients).

[EIP-1193](https://eips.ethereum.org/EIPS/eip-1193) standardizes the way wallets interact with DAPPs. In this EIP, the wallet exposes its interface through the Provider object, and the DAPP interacts with the wallet through the interface provided by the Provider. The functionality of this EIP can be expanded by supporting new RPC APIs. For hosted wallets, the implementation, authorization, supported functions, and plugin wallets may have certain differences, therefore, when compatible with EIP-1193, hosted wallet providers often need to define additional RPC APIs to provide services, and in some cases, they may also modify existing RPC APIs. Different hosted wallets may provide different APIs, which means that developers need to adapt to different APIs for each hosted wallet, increasing development difficulty and workload, and limiting the application range of hosted wallets among developers.

This document aims to propose a set of RPC API specifications for hosted wallets, providing developers with consistent RPC API standards, reducing developers' workload and development difficulty, and promoting the wider application of hosted wallets. These API specifications will cover a series of operations related to hosted wallets, such as authorization, revocation of authorization, signature, etc. In addition, this CIP will also detail the parameters, return values, and exceptional situations of each API, so that developers can better implement or use the APIs. 


## Specification

In this document, some key terms are highlighted in bold to indicate the degree of "requirement", and these words correspond to the relevant specifications in [RFC2119](https://www.rfc-editor.org/rfc/rfc2119).

- MUST. Used to indicate a requirement that must be followed.
- MUST NOT. Used to indicate a requirement that must not be followed.
- SHOULD, RECOMMENDED. Used to indicate a requirement that should be followed in most cases, but can be ignored for specific reasons.
- SHOULD NOT, NOT RECOMMENDED. Used to indicate a requirement that should be followed in most cases, but can be ignored for specific reasons.
- MAY, OPTIONAL. Used to indicate an optional requirement.
...
- Hosted wallet provider. The provider of hosted wallet services, also refers to the website/app controlled by the hosted wallet provider. Users need to interact with the hosted wallet provider to authorize DAPPs. 
...
Authorize an account. After calling this API, the user will interact with the hosted wallet provider to authorize the DAPP. The behavior and parameters of this API depend on the specific implementation.
...
- target: string
  ...
  - Includes the following options, and the hosted wallet provider **may** only support some of them.
    ...
    - `iframe` Open the authorization page in an iframe on the page.
      ...
      > Among these three parameters, blank and self will direct the user to a page controlled by the hosted wallet provider for authorization, while iframe will authorize within an iframe on the page. For more information, see [Appendix 2: target parameter](#appendix-2-target-parameter).

- availableChains:

  - A list of network IDs that users can select, such as `['0x405']`(In Conflux, 1029 is the main network, and 1 is the test network).
...
- scope:
  ...
  - Depends on the services provided by the hosted wallet provider, and determines the types of operations that can be performed after authorization. The hosted wallet provider **must** show the scope requested by the application to the user and obtain their approval.
    - Optional. The hosted wallet provider **may** allow the user to approve only a subset of the scope requested.
  - The hosted wallet provider **must** support `address` and `sign` scopes.
    - `address`. View the address.
    - `sign`. Apply for permission to sign data/transactions. When this permission is not approved by the user, the signature interface will be unavailable. The hosted wallet provider **may** also support the following two sub-scopes to refine the DAPP's signing capability:
      - `sign_data`
      - `sign_tx`
  - Optional. The hosted wallet provider may support other scopes, such as:
    - `phone`
    - `email`
    - `avatar`
    - `openid`
  ...
  > After the `sign` scope is approved, it does not mean that the request for signing no longer requires user authorization. This depends on the implementation of the hosted wallet provider.
...
Sign and send the transaction. When `scope` does not contain `sign` (or sub-permission `sign_tx`), an error **should** be thrown directly. At the same time, the wallet provider **should** assist users in reviewing the transaction in an appropriate manner (such as SMS, mobile apps, etc.). For transactions with `data` that cannot be parsed, the wallet provider can optionally warn the user, reject the signature, etc. to avoid possible security risks.
...
> For additional discussion of this interface, see [Whether to add additional parameters to cfx_sendTransaction](#whether-to-add-additional-parameters-to-cfx_sendtransaction)
...
CIP-23 structured data signature. When `scope` does not contain `sign` (or sub-permission `sign_data`), an error **should** be thrown directly. At the same time, the wallet provider **should** assist users in reviewing the signature content in an appropriate manner (such as SMS, mobile apps, etc.).
...
CIP-23 non-structured message signature. When `scope` does not contain `sign` (or sub-permission `sign_tx`), an error **should** be thrown directly. At the same time, the wallet provider **should** assist users in reviewing the signature content in an appropriate manner (such as SMS, mobile apps, etc.).
...
## Appendix 1: Implementation and Security Recommendations for Custodial Wallet Providers

### Implementation

This CIP does not impose mandatory requirements on "implementation" and "security". Custodial wallet providers can choose their own implementation methods.
...
- Pay attention to the expiration time of the access token. If you need to support the refresh token, it is recommended to support the refresh token rotation.
...
Corresponding to the window in which the user authorizes. The `self` and `blank` parameters are compatible with the OAuth2 process and will open the authorized page controlled by the custodial wallet provider in the current window/new window respectively. After the authorization is completed, the token (code) will be redirected to the specified page (the `redirect_uri` in OAuth2).
...
When `target` is `blank`, the return value can be obtained in the original window through `window.opener` and `window.openee`. However, it should be noted that in specific environments, `window.opener` and `window.openee` may not be available, which may cause the behavior of the `blank` parameter to be inconsistent with expectations.
